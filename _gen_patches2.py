...
with open(os.path.join(base, 'cctv_news-dashboard.html'), 'r', encoding='utf-8') as f:
    cctv_news_dashboard = f.read()
with open(os.path.join(base, 'cctv_people-dashboard.html'), 'r', encoding='utf-8') as f:
    cctv_people_dashboard = f.read()

# Read media_platform configs
with open(os.path.join(base, 'media_platform', 'cctv_news', 'config.py'), 'r', encoding='utf-8') as f:
    news_config = f.read()
with open(os.path.join(base, 'media_platform', 'cctv_people', 'config.py'), 'r', encoding='utf-8') as f:
    people_config = f.read()

# 1. Patch cmd_arg/arg.py
arg_content = arg_content.replace(
    'ZHIHU = "zhihu"',
    'ZHIHU = "zhihu"\n    CCTV_NEWS = "cctv_news"\n    CCTV_PEOPLE = "cctv_people"'
)

# 2. Patch api/schemas/crawler.py
...
patches = {
    ...
    # New files
    os.path.join(base, 'config', 'cctv_news_config.py'): news_config,
    os.path.join(base, 'config', 'cctv_people_config.py'): people_config,
    os.path.join(base, 'api', 'webui', 'cctv_news-dashboard.html'): cctv_news_dashboard,
    os.path.join(base, 'api', 'webui', 'cctv_people-dashboard.html'): cctv_people_dashboard,
}
